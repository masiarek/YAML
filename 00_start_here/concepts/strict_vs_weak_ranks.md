...
- *"Ranked = good, so all ranked methods behave alike."* — They don't. A weak-ranked Condorcet method reliably finds the candidate closest to the center of opinion; strict-rank IRV can eliminate that very candidate ([center squeeze](RCV_IRV/center_squeeze.md)).
...
- [Ranked Robin (Consensus Voting)](./ranked_robin.md) — the Condorcet method that *does* allow equal ranks and compares pairwise
- [Is IRV "just plurality"?](RCV_IRV/irv_and_plurality.md) — why elimination is a first-choice plurality test (and where that claim overreaches)
- [Center squeeze](RCV_IRV/center_squeeze.md) — what strict-rank IRV does to a strong compromise candidate
- [Is RCV "simple"? (201)](RCV_IRV/is_rcv_simple.md)
