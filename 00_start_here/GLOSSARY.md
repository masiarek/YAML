...
- **Quorum** — a *turnout* threshold (separate from who wins): enough of the **eligible electorate** must participate or no winner is declared. Opt-in via `eligible_voters` + `quorum`; default is a majority (>50%) of eligible voters; participation includes abstentions. → page [`quorum.md`](quorum.md); demo [`quorum_demo_c3_b6.yaml`](../01_Single_winner/quorum_demo_c3_b6.yaml)
...
- **STV (Single Transferable Vote)** — the proportional, multi-winner tabulation of ranked ballots (the proportional cousin of IRV, not IRV itself). → demo [`03a_stv_3seats.yaml`](../02_Multi_winner/03a_stv_3seats.yaml); page [`proportional_representation/stv/proportional_stv_vs_star.md`](proportional_representation/stv/proportional_stv_vs_star.md)
...
- **Proportional STAR** — multi-winner methods (Reweighted Range Voting, Allocated Score, Sequentially Spent Score) that give proportional representation. → demo [`03b_star_pr_3seats.yaml`](../02_Multi_winner/03b_star_pr_3seats.yaml); page [`proportional_representation/stv/proportional_stv_vs_star.md`](proportional_representation/stv/proportional_stv_vs_star.md) (STV vs STAR-PR); [`CURRICULUM.md`](CURRICULUM.md) (301.1)
